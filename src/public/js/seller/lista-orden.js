...
const enviarOrden = async (data) => {
  return await fetch("/registrar-orden-vendedor", {
    method: "POST",
    body: JSON.stringify(data),
    headers: {
      "Content-type": "application/json; charset=utf-8",
            ...
            <h3 class="text-light text-center">Orden registrada correctamente</h3>
              <div class="row">
                <div class="col-sm-12 text-center">
                  <img src="/css/assets/check.png"/ width="330px" height="265px">
                </div>
                <div class="col-sm-12 text-center">
...
fetch("/ver-lista-compra", {
    ...
    d.addEventListener("click", e => {
        ...
        if(e.target.textContent == "Eliminar"){
          ...
          if($agregados.textContent == "" || $agregados.textContent < 10){
            d.getElementById("message").innerHTML = `
            <div class="mt-2 alert alert-danger alert-dismissible fade show" role="alert">
              Para registrar una orden de compra debe agregar mas de 10 productos o más. 
            </div>
            `
          }else{
            if($cliente.value == 0){
              d.getElementById("message").innerHTML = `
              <div class="mt-2 alert alert-danger alert-dismissible fade show" role="alert">
              Para poder registrar la orden debe seleccionar un cliente. 
              </div>
              `
